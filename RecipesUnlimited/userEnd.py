...
def optionsPrompt(mustUse, canUse):
    myprompt = ("Provide a 1-2 sentence overview of 5 varied recipes which"
                + " note all the key ingredients once. The suggestions must use "
                + "at least one of these ingredients:\n"
                + mustUse + "\n\nThe recipe can also use any ingredients that are listed below:\n"
                + canUse +"\n\n")
    print(myprompt)
    return myprompt
...
def ideasPrompt(options):
    myprompt = ("Provide 5 varied recipe briefs that use "+
                " or ".join([options]))
    return myprompt
...
def recipePrompt(outline, mustUse, canUse):
    myprompt = ("Write a recipe for this dish: \n\n" + outline
                + "\nthe recipe may include any of these ingredients:\n"
                + canUse + ", " + mustUse + "\n\nAvoid using ingredients that "
                + "aren't typically in the recipe.")
    
    print("\n_______________\n\n\n" + myprompt)
    return myprompt

def recipePrompt2(outline, mustUse, canUse):
    myprompt = ("Write a recipe for this dish: \n\n" + outline)
...
def recipeAdder(recipebook, recipe):
    ...
    if cont:
        recipebook.addRecipe(kitchenManager.Recipe.textToRecipe(recipe))
        print("Recipe added")
    else:
        print("\nRecipe not added.")
...
def modeSelector():
    return int(input("Type 0 for general ideas, press 1 to use the cupboard\n"))

userChoice = modeSelector()

if userChoice == 1:    
    cupboard = kitchenManager.Cupboard("kitchenContents.db")
    canUse = ingredientReturner(cupboard)
    mustUse = perishableAdder(cupboard)
    myprompt = optionsPrompt(mustUse,canUse)
    options = sendPrompt(myprompt, temperature = 0.3)
    ...
    print(recipe)
    recipebook = kitchenManager.RecipeBook("kitchenContents.db")
    recipeAdder(recipebook, recipe)
elif userChoice == 0:
    userInput = input("Write any ingredients that need using: ")
    myprompt = ideasPrompt(userInput)
    print("\n\n"+myprompt)
    options = sendPrompt(myprompt, temperature = 0.6)
    print(options)
