...
# def generate_snake(start_img, position, commands, out_img):
    ...
    comMap = {"S"  : (1,0,0),#(Y change,X change, complex move)
              "E"  : (0,1,0),
              "N"  : (-1,0,0),
              "W"  : (0,-1,0),
              "NW" : (-1,-1,1),
    ...
    for coms in commands:
        #calcs next x and y positions, and next colour
        coms = comMap[coms]
        nextY, nextX = (headY + coms[0]) % SIZEY, (headX + coms[1]) % SIZEX
        nextCol = image[nextY][nextX]
        
        #checks if the snake will collide with itself (unless it's the tip of the tail)
        #or if, the move is a diagonal move, if it crosses a diagonal of itself
        if ( nextCol in [GRN, RED] or
             coms[2] and (image[headY][nextX],image[nextY][headX]) == (GRN,GRN)):
                break # gameover
        
        #removes end of snake if it's not eating
        if nextCol != ORG:
            tailY, tailX = snake.pop(0)
            image[tailY][tailX] = GRY
        
        #updates head position and turns head green
        headY, headX, image[nextY][nextX] = nextY, nextX, GRN
        snake.append([nextY,nextX]) #appends new head position to array
